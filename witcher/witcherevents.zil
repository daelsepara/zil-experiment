...
<ROUTINE RELENTLESS-ASSAULT (MONSTER WEAPON "AUX" KEY)
	...
	<REPEAT ()
		<PERFORM ,V?ATTACK .MONSTER .WEAPON>
		<COND (<NOT <IN? .MONSTER ,HERE>>
			<TELL "... and prevailed! " CT .MONSTER
			<COND (<FSET? .MONSTER ,PLURALBIT> " die.")(ELSE " dies!")> CR>
			<SETG SHOW-COMBAT-MESSAGES T>
			<CHECK-ITEM-DROP .MONSTER ,HERE>
			<SET KEY !\0>
			<RETURN>
		)>
...
<ROUTINE WEAPON-INEFFECTIVE (MONSTER SWORD "AUX" DMG)
	...
	<COND (<L? <GETP .MONSTER P?HIT-POINTS> 1>
		...
		<COMPLETE-IF-QUEST .MONSTER <LOC .MONSTER>>
		<CHECK-ITEM-DROP .MONSTER <LOC .MONSTER>>
		<REMOVE .MONSTER>
		<RETURN>
	)>
...
<ROUTINE QUEST-TALK (QUEST PERSON TEXT GREET REPORT QUEST-TYPE)
	...
	<COND (<VERB? TALK>
		...
		<COND (<NPC-NOT-UNLOCKED .PERSON .GREET> <RTRUE>)>
...
<ROUTINE DESCRIBE-EXIT (DIR-NAME DIR LOC COUNT)
	<COND (<IS-VISIBLE .LOC .DIR> <COND (<G? .COUNT 0> <TELL ", ">)> <COND (<VISITED .LOC .DIR> <HLIGHT ,H-BOLD> <TELL .DIR-NAME ": "> <HLIGHT 0> <TELL D <GETP .LOC .DIR>>)(<TELL .DIR-NAME>)> <RTRUE>)>
	<RFALSE>>

<ROUTINE DESCRIBE-EXITS (LOC "AUX" COUNT)
	...
	<HLIGHT 0>
	<TELL ": ">
	<COND (<DESCRIBE-EXIT "Northwest" ,P?NW .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "North" ,P?NORTH .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)> 
	<COND (<DESCRIBE-EXIT "Northeast" ,P?NE .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "West" ,P?WEST .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "East" ,P?EAST .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "Southwest" ,P?SW .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "South" ,P?SOUTH .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "Southeast" ,P?SE .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "Up" ,P?UP .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<DESCRIBE-EXIT "Down" ,P?DOWN .LOC .COUNT> <SET COUNT <+ .COUNT 1>>)>
	<COND (<0? .COUNT> <TELL " None">)>
	<CRLF>
	<RTRUE>>
...
<ROUTINE CHECK-ITEM-DROP (MONSTER LOC "AUX" ITEM-DROP)
	<SET ITEM-DROP <GETP .MONSTER ,P?ITEM-DROP>>
	<COND (<AND .ITEM-DROP <NOT <IN? .ITEM-DROP ,PLAYER>>>
		<COND (,SHOW-COMBAT-MESSAGES
			<CRLF>
			<HLIGHT ,H-BOLD>
			<TELL "[" CT .MONSTER " drop">
			<COND (<FSET? .MONSTER ,PLURALBIT> <TELL " a">)(ELSE <TELL "s a">)>
			<COND (<FSET? .MONSTER ,VOWELBIT> <TELL "n ">)(ELSE <TELL " ">)>
			<TELL D .ITEM-DROP " here]">
			<HLIGHT 0>
			<CRLF>
		)>
		<MOVE .ITEM-DROP .LOC>
...
<ROUTINE SPAWN-MONSTER (LOC "AUX" MONSTER RESPAWN LASTRESPAWN)
	...
	<COND (.MONSTER
		...
		<COND (<AND <G? .RESPAWN 0> <OR <G? ,MOVES <+ .LASTRESPAWN .RESPAWN>> <0? .LASTRESPAWN>>>
			...
			<COND (<EQUAL? .MONSTER ,DROWNER> <RESET-MONSTER ,DROWNER HP-DROWNER .LOC>)>
		)>
		<RETURN>
	)>>
...
<ROUTINE NPC-NOT-UNLOCKED (PERSON GREETINGS "AUX" UNLOCKED-BY)
...
<ROUTINE NPC-SLEEPING (PERSON)
	<COND (<AND <NOT ,DAYTIME> <FSET? .PERSON ,PERSONBIT>>
		<COND (<NPC-NOT-UNLOCKED .PERSON DEFAULT-GO-AWAY> <RTRUE>)>
